...
def get_first_last_sawing(sgs):
    all_sawing_starts = []
    sawing_counter = []
    for key, pred_sg in sgs:
        scan_idx = int(key.split('_')[1])
        if q(pred_sg, s=['head surgeon'], r=['sawing'], o=['patient']):
...
if __name__ == '__main__':
    predicted_sgs_path = Path('scan_relations_oracle_mv_learned_temporal_pred_test.json')  # TODO adjust as necessary
    ...
    for split_name in ['train', 'val', 'test']:
        for take_idx in TAKE_SPLIT[split_name]:
            ...
            first_sawing, last_sawing = get_first_last_sawing(sgs)

            for key, pred_sg in sgs:
                scan_idx = int(key.split('_')[1])

                # sterile -> roll_in
                if current_phase == 'sterile' and (q(pred_sg, s=['patient']) or q(pred_sg, o=['patient'])) and q(pred_sg, r=['manipulating'],
                                                                                                                 o=['operating table']):
                ...
                elif current_phase == 'roll_in' and q(pred_sg, s=['head surgeon'], r=['preparing']) and q(pred_sg, s=['assistant surgeon'], r=['preparing']):
                ...
                # patient_prep -> knee_prep with a sanity check: Cleaning Done
                    ...
                    if not memory['cleaning_done']:
                        if q(pred_sg, s=['head surgeon', 'assistant surgeon'], r=['cleaning'], o=['patient']):
                        ...
                        if q(pred_sg, s=['head surgeon', 'assistant surgeon'], r=['cutting']):
                ...
                # knee_prep -> knee_insert with a sanity check: Sawing and Drilling Done
                    ...
                    if scan_idx > last_sawing and q(pred_sg, s=['head surgeon'], r=['hammering'], o=['patient']):
                ...
                # knee_insert -> surgery_conclusion with a sanity check: Cementing Done
                    ...
                    if not memory['cementing_done']:
                        if q(pred_sg, s=['head surgeon', 'assistant surgeon'], r=['cementing'], o=['patient']):
                        ...
                        if q(pred_sg, s=['head surgeon', 'assistant surgeon'], r=['suturing'], o=['patient']):
                ...
                elif current_phase == 'surgery_conclusion' and (q(pred_sg, s=['patient']) or q(pred_sg, o=['patient'])) and q(pred_sg, r=['manipulating'],
                                                                                                                              o=['operating table']):
                ...
                # roll_out -> cleanup
                elif current_phase == 'roll_out' and not q(pred_sg, o=['patient']) and q(pred_sg, s=['circulator', 'anaesthetist']):
